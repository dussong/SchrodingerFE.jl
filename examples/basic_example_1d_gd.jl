...
a = 1.4
b = -1.4
vext(x) = -(3.0 ./ sqrt.((x - a) .^ 2 .+ 1)
            .+
            1.0 ./ sqrt.((x - b) .^ 2 .+ 1)
)
# vext(x) = -(4.0 ./ sqrt.((x) .^ 2 .+ 1))
...
# E_FCIsparse, wf_FCIsparse = WaveFunction(ne, ham, "FCI_sparse"; maxiter=20, kdim=50)
# E_CDFCIsparse, wf_CDFCIsparse = WaveFunction(ne, ham, "CDFCI_sparse"; max_iter=500000, k=500)
...
# ρ_FCIsparse = Main.PairDensities.density(wf_FCIsparse, ham)
...
P3 = contour(xx, xx, ρ2_FCIsparse, c=:darktest, aspect_ratio=:equal,
   colorbar=:true, fill=:true, tickfontsize=16, colorbar_tickfontsize=16)
P3 = title!("Energy=$E_FCIsparse")

savefig("LiH_rho2.pdf")

# P3 = contour(xx, xx, ρ2_CDFCIsparse, c=:darktest, aspect_ratio=:equal,
#    colorbar=:true, fill=:true, tickfontsize=16, colorbar_tickfontsize=16)
