...
export SCI_matfree, SCI_matfree_nbd
# PARAMETERS
...
function SCI_matfree(Ψinit::WaveFunction_sp, ham::Hamiltonian, k::Int64; max_iter=1000, ϵ=5.0e-7, tol=1.0e-3)
    ...
    n1 = binomial(2dof, ne)
    n2 = (2dof)^ne
    @time b, d = ham_column(ne, combBasis, Ψ, ham) # H * c, M * c
    M1 = dot(c, d)
    H1 = dot(c, b)
    energy = H1 / M1
    Dgk = zeros(k)
    ...
    for t = 1:max_iter
        ...
        if length(c.nzind) > k
            ...
            j0 = map(x -> num_s2ns(2dof, ne, x), j0)
            j = vcat(c.nzind, j0)
        end
        l = Int64[]
        for i = 1:k
    ...
    #return r, c#, step;
    #return y,num;
    return y, num, c/norm(c)
end


function SCI_matfree_nbd(Ψinit::WaveFunction_sp, ham::Hamiltonian, k::Int64, H1::Float64, M1::Float64; max_iter=1000, tol=1.0e-3)
